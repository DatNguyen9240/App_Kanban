...
export const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  return (
    <div className={`space-y-2 ${className}`} onPaste={handlePaste}>
      ...
      {value && showPreview ? (
        <div className="relative group rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 bg-slate-900/5 dark:bg-slate-900/50 max-h-64 min-h-[160px] flex items-center justify-center">
          ...
          />
          <div className="absolute inset-0 bg-slate-900/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 z-20">
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              className="px-2.5 py-1.5 rounded-lg bg-white/90 hover:bg-white dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-100 text-xs font-semibold shadow-xs flex items-center gap-1.5 transition-colors"
            >
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-4 text-center cursor-pointer transition-all ${
            isDragging
              ...
              : 'border-slate-200 dark:border-slate-800 hover:border-indigo-300 dark:hover:border-indigo-500/60 bg-slate-50/60 dark:bg-slate-900/60 hover:bg-indigo-50/20 dark:hover:bg-indigo-950/20'
          }`}
        >
          <input
            type="file"
          ...
          {isUploading ? (
            <div className="py-2 flex flex-col items-center gap-2">
              ...
              <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">Uploading image to server...</p>
            </div>
          ) : (
            <div className="py-1 flex flex-col items-center gap-1.5">
              <div className="w-9 h-9 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 flex items-center justify-center border border-indigo-100 dark:border-indigo-900/60">
      ...
      {error && (
        <p className="text-[11px] font-medium text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/40 border border-rose-100 dark:border-rose-900/50 px-2.5 py-1.5 rounded-lg flex items-center justify-between">
          <span>{error}</span>
          <button
            type="button"
            onClick={() => setError(null)}
            className="text-rose-400 hover:text-rose-700 dark:hover:text-rose-200 ml-2"
          >
            <X className="w-3 h-3" />
          </button>
        </p>
